...
def main(args):
    ...
    MAX_WORKES = args.max_workers
    OUT_SIZE = tuple(args.out_size)
    OUT_SHAPE_2D = tuple(args.out_shape_2d)
    SPACING_2D = args.spacing_2d
    OUT_SIZE_MM_2D = tuple(np.array(OUT_SHAPE_2D) * SPACING_2D)
    MIN_MM2 = args.min_mm2
    WRITE_NORESAMP_NODULES = args.write_noresamp_nodules
    DO_RESAMP = args.do_resamp
    DO_SLICES = args.do_slices
    ...
    if RESTART or (not (DATA_DIR / "scan_df.csv").exists()):
        scan_df_dict = {}
        ...
        scan_df.to_csv(DATA_DIR / 'scan_df.csv', index=True, index_label="patient_id")
    else:
        scan_df = pd.read_csv(DATA_DIR / 'scan_df.csv')
    ...
    for patient_id, scan in scan_dict.items():
        if patient_id in patients_with_nodules:
            ...
            try:
                ...
                print("Clustering failed for patient {}; skipping".format(patient_id))
            except Exception as e:
                print("An unexpected error occurred for patient {}; skipping".format(patient_id))
                print(e)
    ...
    if RESTART or (not (DATA_DIR / "annotation_df.csv").exists()):
        ...
        annotation_df.to_csv(DATA_DIR / "annotation_df.csv", index=False)
    else:
        annotation_df = pd.read_csv(DATA_DIR / "annotation_df.csv")
    ...
    if WRITE_NORESAMP_NODULES: 
        ...
        for nodule_id, annotation_list in tqdm(clustered_annotations.items()):
            for i, ann in enumerate(annotation_list):
                annotation_id = "{}a{}".format(nodule_id, i+1)
                if RESTART or (not os.path.exists(os.path.join(DATA_DIR, "nodules3d-noresamp", "{}.npy".format(annotation_id)))):
                    try:
    ...
    out_dir2d = Path(DATA_DIR) / "nodules2d"
    out_dir2d.mkdir(parents=True, exist_ok=True)
    (out_dir2d / "masks").mkdir(parents=True, exist_ok=True)
    (out_dir2d / "imgs").mkdir(parents=True, exist_ok=True)

    out_dir3d = Path(DATA_DIR) / "nodules3d"
    out_dir3d.mkdir(parents=True, exist_ok=True)
    (out_dir3d / "masks").mkdir(parents=True, exist_ok=True)
    (out_dir3d / "arrs").mkdir(parents=True, exist_ok=True)
    ...
    if DO_RESAMP: 
        ...
        for ann_id, ann in tqdm(anns.items()):
            ...
            if RESTART or (not os.path.exists(out_dir3d / "arrs" / (ann_id+".npy"))):
    ...
    # 
    existing_files = os.listdir(out_dir2d / "imgs")
            ...
            if (not ann_id in existing_slices) or RESTART:
                ...
                for slice_index in range(num_slices):
                    ...
                    if mask_i.sum() > (MIN_MM2 / (scan_spacing**2)):
                        ...
                        img_nod.save(out_dir2d / "imgs" / (slice_id+".png"))
                        img_mask = Image.fromarray(mask_i, mode="L")
                        img_mask = img_mask.resize(OUT_SHAPE_2D[:2])
                        img_mask.save(out_dir2d / "masks" / (slice_id+".png"))

    file.close()

if __name__ == "__main__":
    import argparse
    parser = argparse.ArgumentParser(description="LIDC preprocessing script")
    parser.add_argument('--data-dir', type=str, default="../data", help='Data directory')
    parser.add_argument('--log-file', type=str, default="log/lidc-preprocessing.log", help='Log file path')
    parser.add_argument('--max-workers', type=int, default=1, help='Max workers')
    parser.add_argument('--out-size', type=int, nargs=3, default=[69,69,69], help='3D output size')
    parser.add_argument('--out-shape-2d', type=int, nargs=2, default=[180,180], help='2D output shape')
    parser.add_argument('--spacing-2d', type=float, default=0.5, help='2D spacing')
